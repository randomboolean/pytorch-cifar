...
TRANSLATIONS_PATH_CROP="translations/translations_symmetric"
...
class GraphCrop(object):
    ...
    def __init__(self,translations_path=TRANSLATIONS_PATH_CROP,times_to_translate=4,repetitions=2):
        self.translations = ast.literal_eval(open(translations_path, "r").read())
        self.inverted_translations = [dict([v,k] for k,v in translation.items()) for translation in self.translations]
    ...
    def shift_direction(self,image,direction):
        ...
        directions = (torch.zeros(1024)-1).type(torch.LongTensor)
    ...
    def __repr__(self):
        return self.__class__.__name__ + '(translations={0})'.format(self.translations)
...
class GraphPreActBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, number_of_strides,stride=False,translations_path=TRANSLATIONS_PATH):
        ...
        self.conv1 = TernaryLayer(in_channels=in_planes, number_of_strides=number_of_strides, out_channels=planes)
        self.bn2 = nn.BatchNorm1d(planes)
        self.conv2 = TernaryLayer(in_channels=planes, number_of_strides=number_of_strides,out_channels=planes)
...
class GraphPreActResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.conv1 = TernaryLayer(in_channels=3, out_channels=64)
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for stride in strides:
            ...
            layers.append(block(self.in_planes, planes, self.number_of_strides,stride))
...
def GraphPreActResNet18():
    return GraphPreActResNet(GraphPreActBlock, [2,2,2,2])
...
class TernaryLayer(torch.nn.Module):
    ...
    def build(self):
        ...
        for translation in lines:
            for vertex in translation:
                indices[i, (translation[vertex] - 1) % self.kernel_size] = vertex + 1 
                counts[i] += 1
            i += 1

        self.indices = torch.LongTensor(indices).cuda()
